...
rb.ui.dialog = { 
	create : function(call, winTitle, winContentWidth, winContentHeight){
		...
		$('<div id="rbWindowContent" class="modal fade loading" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>') 
		...
		$('#rbWindowContent').on('shown.bs.modal', function () {
			var modal = $(this);
		...
		$('#rbWindowContent').on('hidden.bs.modal', function() {
			$('#rbWindowContent').remove();
		});
